...
async function generateUniqueUsername(firstName?: string | null, lastName?: string | null): Promise<string> {
...
export async function findOrCreateOAuthUser(provider: OAuthProviderId, profile: OAuthProfile): Promise<User> {
  ...
  if (existingAccount) {
    ...
    if (!user) throw new Error("Linked OAuth account has no matching user.");
    return user;
  }
