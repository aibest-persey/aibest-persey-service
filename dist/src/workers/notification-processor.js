...
async function recoverStaleJobs() {
    ...
    const [count] = await NotificationJob.update({ status: "pending", processAfter: new Date() }, { where: { status: "processing", updatedAt: { [Op.lt]: threshold } } });
    if (count > 0) {
...
async function processNextJob() {
    ...
    try {
        ...
        if (job.attempts >= MAX_ATTEMPTS) {
            ...
            job.status = "failed";
            console.error(`[Processor] Job ${job.id} (${job.type}) permanently failed after ${job.attempts} attempts:`, job.lastError);
        }
        else {
            // Schedule retry with exponential backoff
            job.status = "pending";
            job.processAfter = new Date(Date.now() + backoffMs(job.attempts));
            console.warn(`[Processor] Job ${job.id} (${job.type}) failed ` +
