...
const getEventFormContext = async (userId, role) => {
    if (role === "admin") {
...
export const createEvent = async (req, res) => {
    try {
        const parsedPayload = await parseCreateEventPayload(req, req.body);
        if ("error" in parsedPayload && parsedPayload.error) {
            res.status(400).json({ message: parsedPayload.error.join("; ") });
            return;
        }
        const { title, description, coverImage, startAt, endAt, capacity, visibility, ownerScope, } = parsedPayload.values;
        const agenda = req.body.agenda ?? null;
        const organisationId = req.body.organisationId;
        let organisationIdValue = null;
...
export const getEventFormOptions = async (req, res) => {
    try {
        const context = await getEventFormContext(req.user.id, req.user.role);
...
export const updateEvent = async (req, res) => {
    try {
        ...
        event.agenda = req.body.agenda ?? null;
...
export const cancelEvent = async (req, res) => {
    try {
        ...
        await sequelize.transaction(async (t) => {
            ...
            await Registration.update({ status: "cancelled", waitlistPosition: null }, { where: { eventId: event.id, status: "waitlisted" }, transaction: t });
        });
        res.json(event);
...
export const registerForEvent = async (req, res) => {
    ...
    try {
        const registration = await sequelize.transaction(async (t) => {
            ...
            if (event.status === "cancelled")
                throw Object.assign(new Error("cancelled"), { code: 410 });
...
export const cancelRegistration = async (req, res) => {
    ...
    try {
        await sequelize.transaction(async (t) => {
            ...
            if (!event)
                throw Object.assign(new Error("event_not_found"), { code: 404 });
        ...
        if (cancellingStudent) {
            eventBus.publish("registration.cancelled", {
                ...
                eventTitle,
                studentId,
                studentEmail: cancellingStudent.get("email"),
        ...
        if (promotedStudentId && promotedRegistrationId && promotedStudent) {
            eventBus.publish("registration.promoted", {
                ...
                studentEmail: promotedStudent.get("email"),
