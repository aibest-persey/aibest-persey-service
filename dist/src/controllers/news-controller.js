...
const canModifyNews = async (news, userId) => {
...
const canAccessNews = async (news, userId, userRole) => {
    if (news.scope === "public")
        return true;
    if (userRole === "admin")
        return true;
    if (news.scope === "org" && news.organisationId) {
...
export const createNews = async (req, res) => {
    try {
        const { title, content, scope, organisationId, clubId } = req.body;
...
export const listNews = async (req, res) => {
    try {
        const { scope, organisationId, clubId } = req.query;
        const filters = {};
        if (scope) {
